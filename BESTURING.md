...
| `-EclipseShot -EclipseShotFixtures` | dezelfde review-camera's, maar met **alleen de armaturen** zichtbaar (alle andere actoren verborgen). Gebouwd omdat een oordeel over "de lampen" anders over de verkeerde objecten kan gaan: op het volle frame stonden twintig ambervlakken en er zijn er dertien geplaatst. Wat hier in beeld staat, ZIJN ze — en het aantal wordt gecontroleerd |
| `-EclipseShotPlay` | de **speelronde met opnames**: negen momenten vanuit de speler (stilstaand, lopend, vurend, isolatie, twee draaimomenten, met UI, midden in het herladen), elk met een meting van schaal, silhouethoogte in pixels, afstand en of de renderer het lichaam getekend heeft. Draai hem via `Eclipse\Tools\verify.ps1`, dat ook bouwt, de suite draait, valideert en de catalogus controleert |
...
| `Eclipse.UI.Report` | **wat er op het scherm staat, zonder screenshot.** Panelen, zichtbaarheid en regelaantallen. Dit bestaat omdat de UI-laag met géén enkele opnamemethode vast te leggen is — HighResShot, `FScreenshotRequest` met `bShowUI` en een Slate-vensteropname leveren alle drie hetzelfde 3D-beeld zonder widgets. "Ik zie hem niet" is dus geen bewijs; dit commando is dat wel. Meldt ook zelf een FOUT bij een HUD buiten de viewport, een verborgen munitieteller terwijl je een wapen draagt, of een paneel dat open staat met nul regels |
| `Eclipse.Save.Report` | **waarom is mijn kaart leeg?** Eerst of de campagne zijn setup-asset kent — zonder dat is er wél een campagne maar géén regiograaf, en dan biedt geen enkel vak een missie aan — dan de staat (dag, story-beats, regio's) en dan de slots op schijf met hun grootte. In die volgorde, want een rapport dat met de slots begint laat de helft weg die op 27-07 stukging: de save kwam bit-voor-bit terug en het bord bleef leeg. Leest bewust alleen wat er nu al is; de content-identiteit ín het bestand staat in SPEC-P2-06 en wacht op review |
| `Eclipse.Liberation.Report` | **waarom draaide er geen regio om?** Eerst of de tabel aan `DA_CampaignSetup` hangt — een rapport dat met "0 rijen" begint laat in het midden of hij leeg is of niet gevonden — dan per rij de trigger, de poort en de huidige eigenaar van elk vak. Dit had de bug van 27-07 in één regel verklapt: de Foothold-rij stond klaar, de logica was tien keer unit-getest, en de tabel was simpelweg niet gekoppeld |
