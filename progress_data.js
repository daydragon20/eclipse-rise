...
window.PROGRESS_DATA = {
  ...
  taken: [
    ...
    { taak: "Pack-slim-ronde (migrate + drop)", status: "gepland", pct: 0, detail: "Vervolg op de opruimronde (owner-vraag 23-07): ÁLLE curatie-accepts migreren naar /Game/Art/Imported — niet alleen de al-geplaatste (well/ring + SciFi10-slots 1/2) maar ook de accepted-unplaced A1-plinth, A3-slagkei, C1-brokstuk en SciFi10-albedo's 5/6/7/9/10 (anders verliest de dressing-ronde zijn bronnen bij de pack-drop). Builder-paden omzetten, dan Minions+SciFi10 van schijf — ~5,9 GB vrij. Eerste in de commandlet-rij na de P2-01-tests." },
    { taak: "P2-02 Stage A: Command Mode debug-feel (R3)", status: "gepland", pct: 0, detail: "Direct na de P2-01-merge (main): hold-to-enter + 0.30-dilatatie (wrapper-component met fail-safe naar 1.0) + per-soldier selectie op de bestaande 4 verbs, debug-HUD only — de R3-falsificatie op de lelijkste build (SPEC-P2-02 locked decisions 1-3; 8.4/4.1.2/9.5). DA_CommandModeTuning (geen hardcoded getallen), ack-meting op wall-clock, Event.Command.ModeEntered/Exited + catalog zelfde commit, Gauntlet 20× enter/exit dilatatie exact 1.0, scenario 'order under fire at 0.30' ≤1 s. Daarna feel-gauntlet als owner-touchpoint; Stage B pas ná verdict 'true'." },
    { taak: "P2-03 stap 1-2: DT_Facilities + EclipseBaseLogic headless", status: "gepland", pct: 0, detail: "Parallel aan P2-02 Stage A (tweede agent, geen file-botsing: Base/+Strategy/ vs Squad/): FEclipseFacilityRow/DT_Facilities, UEclipseBaseLayoutAsset (slot-graph A-D), DA_BaseTuning, FEclipseBaseState in FCampaignState mét migratie-entry + v0-fixture-test in hetzelfde commit (14.3.6, R6), en pure EclipseBaseLogic (build-validatie, day-tick, crew -1, rush) met deterministische unit tests — geen engine-actor-headers (14.3.2). Asset-materialisatie (setup-script) schuift in de commandlet-rij; wrapper/events/vault = volgende iteratie." },
    { taak: "15.8 dressing-ronde: rubble/warehouse + watch-items", status: "gepland", pct: 0, detail: "NA pack-slim (zelfde builder-lock, serieel): A3-slagkei + C1-brokstukken (3-5× geïnstanced ónder A3) als Contested-rubble tegen de perimeterwal, A1-plinth als loading-dock in de warehouse-yard; SciFi10-slots 5/6/7/9/10 (warehouse-binnenwand, machine-bank, BldgA-trim, treadplate-ramps, gantry-grid) en de 3 ongeplaatste ambientCG-albedo's (Metal046B 19.76, Concrete042A 8.94, Metal063 6.42) op hun curatie-bestemmingen — gains al gemeten, niet schatten. In dezelfde ronde de watch-items: teal-fin cam 6, perimeterband cam 4, crosshatch-tiling + horizon-audit. Shotronde + art-review per 15.8 vóór commit." },
    { taak: "Wachtrij eigenaar", status: "wachten", pct: 0, detail: "1) MetaHuman-items: 1× Download-klik per item in Window → Fab → My Library (login werkt) · 2) MH_<Naam>-gezichten per phase0/metahuman_recipes.md · 3) env-pack-pulls: Factory Pack Vol.1, Industrial Building 49 PBR, UNIBLOCKS, Sci FI Hallway, Sci-Fi Light Pack, Auto Footsteps Utility, Niagara Footstep VFX, FPS Weapon Bundle, Free Muzzle Flash · 4) CHARACTER-GAT (enige!): 1 gestileerd civilian/worker-pack, 4–6 bodies, Mannequin-rig, mét arbeider-varianten — voor Hollow Point-crew/idlers (SPEC-P2-03) en Kessara-burgers close-up · 5) 13.2-playtest (~30 min, stuurt R1) · 6) straks: P2-02 Stage A feel-gauntlet (~20 min, het R3-verdict — komt in de rij zodra Stage A speelbaar is). KLAAR: Fab-login ✓, ElevenLabs-scopes ✓, Blender ✓, westgevel-A/B beslist (B, doorgevoerd 412f14f) ✓, GameAnimationSample mag weg (0 refs)." }
  ],
