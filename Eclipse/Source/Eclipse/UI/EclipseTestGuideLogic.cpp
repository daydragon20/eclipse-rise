	...
	namespace
	{
		...
		const FGuideControlDetail GuideControlDetails[ControlStepCount] = {
			...
			{ EEclipseGuideSignal::SelectNext,
			  TEXT("de regel 'target:' springt van ALL naar een soldaat-id, en bij elke druk naar de volgende") },
			{ EEclipseGuideSignal::SelectPrev,
			  TEXT("'target:' loopt de andere kant op — door de squad lopen moet in beide richtingen werken") },
			{ EEclipseGuideSignal::DirectPick,
			  TEXT("de soldaat onder je kruis wordt 'target:'; wijs je niemand aan, dan blijft de vorige selectie staan") },
			...
			{ EEclipseGuideSignal::Stance,
			  TEXT("'stance:' wisselt ready <-> aggressive. Alleen de pad-Y is detecteerbaar: op toetsenbord is Alt een modifier bij de order zelf, dus druk daar J") }
		};
