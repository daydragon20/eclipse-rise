...
bool FEclipseAudioSubsystemBusContractTest::RunTest(const FString& Parameters)
{
	...
	TestEqual(TEXT("Acht live subscriptions: sting + ack + refused + schot + treffer + MISSER + herladen + wapenwissel"), Bus->GetSubscriptionCount(), 8);

	// Re-bind must swap, not stack — a leaked second subscription would double
	// every sting.
	Audio->BindToBus(*Bus);
	TestEqual(TEXT("Re-bind does not leak a subscription"), Bus->GetSubscriptionCount(), 8);
