...
| `Event.Base.ConstructionStarted` | `FEclipseBaseEventPayload` (slot id, facility id, target level, ETA day) | CampaignSubsystem (commit: build order accepted) | Vault presentation (scaffold state on), base/debug UI, audio | P2-03 | implemented |
