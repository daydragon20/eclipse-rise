...
**Bar bij elke commit: build ✓ (-NoUba) · de VOLLEDIGE testsuite groen (0 fail) · EclipseValidateData 4 validators / 0 fouten · catalog gedocumenteerd = geïmplementeerd.** Het exacte testaantal staat bewust niet meer in deze regel — het groeide vannacht met tientallen en de regel liep er drie keer op achter. Het staat wél in de commit-message van elke ronde, waar het niet kán verouderen. Alles tussen `26edd65` en `HEAD` is van deze nacht, alles gepusht — `git log 26edd65..HEAD` is de lijst, en dat is met opzet de enige lijst. Hier stonden eerst zeventien hashes opgesomd; die opsomming liep achter zodra er een commit bijkwam en las tóch als compleet. Dezelfde valkuil als de tellingen die hieronder drie keer beschreven staan, en de oplossing is dezelfde: verwijs naar de bron in plaats van hem over te schrijven. In thema's: het harnas (laag 1 + 2) · de speelronde · S1/S2/S3 · de feel-audit (remmen, richting, camera-probe, sprong-vergevingsvensters, traversal, pitch-demping) · de navigatie · drie bewakers op de beschrijvingen.

*De build is om 03:45 ook echt **gestart** — geen automatiseringsharnas maar `-game -EclipseStartMission=TransitCheckpoint`, dus dezelfde weg die jij neemt. Uit die draai: district gebouwd (13 blokken, 20 dekkingen, 10 sites, 3 ingangen) · navigatiegrens 28000 × 28000 × 4000 · missieactoren gespawnd (3 squadmates, 4 vijanden) · `Mission 'MT_M11' started` · navmesh onder de speler NEE bij start en JA vijf seconden later, precies zoals bedoeld · **nul fouten en nul waarschuwingen van Eclipse**. Dat is geen speelbewijs — ik kan niet zien hoe het voelt — maar het bewijst wel dat alles van vannacht samen opstart, en dat is met een gewijzigde constructor, capsule en camera niet vanzelfsprekend.*

*En de suite kost **38 seconden**, inclusief het opstarten van de engine — gemeten, want jouw opdracht zegt dat de speelronde elke ronde moet meelopen "zoals de testsuite", en dat is alleen vol te houden als het goedkoop is. Zes speelrondes die echte missies starten, 125 tests, ruim een halve minuut. Er is dus geen enkele reden om hem over te slaan.*

*De suite is aan het eind drie keer achter elkaar gedraaid: **123/123, drie keer nul fouten**, en de tijdgevoelige metingen kwamen er identiek uit — 0,100 s voor de camera-overgang, 0,258 s voor de Command Mode-blend, 212,557 cm luchtsturing, 0,150 s sprint-oploop, 7,000 schoten/s. Dat is geen toeval maar constructie: het harnas tikt met een vaste stap, dus het meet reproduceerbaar in plaats van bij benadering. **Elk getal in dit rapport is dus een waarde en geen schatting** — verandert er morgen iets aan zo'n getal, dan is dat een echte verandering en geen ruis.*

*Om 04:25 nog een keer gedaan, nu ook voor de getallen die ná 02:40 zijn bijgekomen — en ook die kloppen: Command Mode-uitzoom 73,333% · camera bij +70° 84,244 cm · luchtsturing 212,557 cm · gehurkte capsule 124,000 cm · 360° tijdens mikken 4,292 s · wapenbereik 4700,000 cm · eerste treffer op 2442,204 cm · van treffer tot neer 2,500 s · dichtste nadering 4169,668 cm. Vier keer heb ik deze controle vannacht gedaan; de eerste twee vonden een fout, de laatste twee niets.*
...
- **Wat er WEL onder zat, en dat is een echt punt voor je balans.** Mijn speelronde schakelt de vijanden uit vanaf **4200 cm**, en hun waarnemingsbereik is **2500–3000 cm**. Het spelerwapen (AR_Foundry) reikt **5000 cm**. Je kunt dus elke vijand in deze missie neerhalen van buiten zijn waarnemingsbereik, zonder dat hij ooit merkt dat er gevochten wordt — en mijn geautomatiseerde speler doet dat ook, want het is de enige manier om met 100 HP tegen vier schutters te winnen.
...
| **Zes overgebleven engine-defaults heb ik NIET aangeraakt**, maar wel zichtbaar gemaakt (`Eclipse.Feel.Layer1.WhichEngineDefaultsAreStillUnchosen`). | Een default kán de juiste waarde zijn — hij is alleen niet gekozen, en dat verschil hoort leesbaar te zijn in plaats van dat "nooit bekeken" als "bewust zo" leest. Vijf van de zes zou ik laten staan: `WalkableFloorAngle` (44,8° is een normale helling), `GravityScale` (1,0, en de sprongboog is al getuned via `JumpZVelocity`), `LedgeCheckThreshold` (4 cm), en de twee `AirControlBoost`-waarden (die verdubbeling onder 25 cm/s helpt je van een richel af stappen en is engine-ontwerp, geen ongeluk). De zesde, **`JumpMaxHoldTime = 0`, is een echte keuze**: langer drukken springt niet hoger. Voor een aardse tactische shooter is dat verdedigbaar — Gears en The Division hebben ook geen vrije variabele sprong — dus mijn advies is laten staan, maar je moet weten dát het zo staat. |
...
| 3 | **Command Mode trekt de camera 73% terug (300→520), maar `DA_CommandModeTuning` zegt 15% en de GDD 4.1.1 ook.** **LET OP — deze vraag is veranderd:** je hebt tot vannacht NOOIT enige uitzoom gezien, want de camera-blend draaide niet (zie §1). Nu hij loopt is hij gemeten: **300 → 520 cm, dus 73,3%**, en die uitzoom kost **0,26 seconde echte tijd** — niet de 85 ms die de blendsnelheid suggereert, want Command Mode zet de wereld op 0,30 en de camera erft die vertraging. Hoe ver én hoe traag horen bij elkaar in je oordeel. Dat is precies wat de code bedoelde, en het is nu voor het eerst iets waar je een oordeel over kunt vellen. Beoordeel hem met de camera aan het werk voordat je een getal kiest. Eén waarheid kiezen. | **Houd de 73% en corrigeer de GDD-regel** via change management. 520 is bewust geauthord om het veld te lezen; 15% (=345) is nooit gespeeld. Maar dit is smaak, dus jouw call. |
...
| 9 | **Bereikasymmetrie.** Jouw wapen reikt 5000 cm, de vijand ziet je pas op 2500–3000. Buiten hun bereik is het risicoloos; binnen hun bereik lig je in **2,50 s** neer. | **Speel eerst één gevecht.** Beide getallen zijn nu gemeten en het zijn er twee die dezelfde keuze beschrijven. Mijn voorkeur als je niets wilt uitproberen: waarneming omhoog naar ~4000, want dat raakt alleen de vijand en niet jouw wapengevoel. |
...
| 13 | **Vier knoppen doen niets buiten Command Mode** (stance, volgende, vorige, onder-kruis). De tabel zegt dat nu; het gedrag is niet aangeraakt. | **Waarschijnlijk zo bedoeld — dan hoef je niets te doen.** Verwacht je dat "volgende" en "onder kruis" ook in het veld werken om snel iemand te selecteren, dan is dat een kleine wijziging. |
| 14 | **Een neergeschoten soldaat zegt "No route, boss."** De reden klopt in de data (Downed), maar de zin komt uit de pool van het ordertype en wijst je dus naar een routeprobleem. | **Eén pool voor Downed-zinnen is genoeg** — die reden geldt voor alle vier de orders identiek. Zeg ja en ik bouw het veld in `DT_SquadOrderDefs`, de validator en de test; de zinnen zelf schrijf jij of ik, zoals je wilt. |
| 15 | **Kopschoten doen niets extra** — 22 hp op borst én hoofd. De ×2,5 staat in `DT_Weapons` maar de trace raakt de capsule, en die heeft geen bones. Structureel, niet tijdelijk. | **Haal de vermenigvuldiger er nu uit** zodat niemand erop rekent, en bouw een hoofd-hitbox wanneer het gevecht aan de beurt is. Dit halveert je time-to-kill zodra het werkt, dus beantwoord het samen met vraag 9. |
| 16 | **Het alarm kan tijdens spelen niet afgaan.** Vier vijanden zien je en schieten je neer — de latch blijft uit. De enige aanroep is een console-commando. | **Koppel hem aan eerste contact van een vijand die je waarneemt.** Die waarneming bestaat al (gemeten op 2442 cm) en het is een regel die je kunt uitleggen. Let op: zodra dit werkt verlies je spook-optionals bij elke detectie, dus de missie wordt strenger. |
...
| 19 | **Acht ingesproken squad-zinnen worden nooit afgespeeld.** De opnames en het stemsysteem bestaan allebei; alleen speelt niets ze af. | **Doen, met een rem van 1–2 s per soldaat.** Dit is klein werk — de audio-subsystem is al een bus-consument, dus het is er één bij op `OrderAcknowledged` en `OrderRefused`. De rem is de enige echte keuze (vier orders achter elkaar = vier stemmen over elkaar). Van alles wat er ligt is dit de goedkoopste stap richting "het gevecht voelt ergens naar". **Nagemeten addertje:** de opnames zijn geschreven tegen een andere woordenlijst dan de code — `NoRoute` en `Down` passen, `NoShot` hoort vermoedelijk bij `NoLineOfSight`, maar `Pinned` past bij géén enkele weigerreden en er ontbreken er twee (een weigering wegens `InvalidTarget`, en een bevestiging op `FocusTarget`). Reken dus op twee zinnen schrijven en één wees opruimen. |
