...
- **De squad weigert ELKE verplaatsingsorder met `NoRoute`** (mét bark: "No route, boss.", "Can't get there from here.", "That path's blocked."). Systeemtechnisch is dat precies goed — elke order krijgt exact één antwoord en de weigering is beredeneerd, nooit stil — maar het betekent dat de squad nergens heen kan. **Ik heb dit uitgezocht en de oorzaak gemeten, en hij is groter dan hij leek.**
  ...
  **Wat er nog over is:** de resterende oorzaak zit tussen de grens en de tegels — invoker-registratie, of de brush van een runtime-gespawnde `ANavMeshBoundsVolume` die niet als volwaardige grens telt. Dat is de eerstvolgende bouwstap, en hij is nu goedkoop: de twee logregels zeggen meteen of een poging werkt.
...
| 4 | **De squad kan nergens heen** (elke order → `NoRoute`). Uitgezocht tot het einde: in een échte draaiende `-game`-run is er ook vijf seconden na missiestart geen navmesh. De grenzen-fix was nodig maar niet genoeg. | **Niets te beslissen, dit is werk van mij.** Ik heb het niet afgemaakt omdat de resterende oorzaak (invoker-registratie of de brush van een runtime-gespawnde grens) meer is dan ik vannacht nog met zekerheid kon landen. De diagnose is nu goedkoop: twee logregels bij elke missiestart zeggen meteen of een poging werkt. |
